...
import domain.Gongji2;

public class GongjiDaoImpl2 implements GongjiDao2 {

	@Override
	public int makeData() { // 테이블 생성 및 값 대입
		try {
			Class.forName("com.mysql.cj.jdbc.Driver"); // jdbc드라이버 로드하여 데이터 베이스와 연결
			Connection conn = DriverManager.getConnection("jdbc:mysql://192.168.23.148:33060/kopo31", "root", "kopo31"); // DB
																															// 연결
			...
			for (int i = 1; i <= 150; i++) { // 125개 데이터 생성
				String title = "title " + i; // 제목 데이터
				java.sql.Date date = java.sql.Date.valueOf("2023-06-13"); // 날짜 데이터
				String content = " 내용" + i; // 내용 데이터
	...
	@Override
	public List<Gongji2> selectAllGongji() {
		List<Gongji2> selectAll = new ArrayList<Gongji2>(); // 공지 리스트 생성

		try { // 오류를 방지하기 위한 try문 시작
			Class.forName("com.mysql.cj.jdbc.Driver"); // MySQL JDBC 드라이버 클래스를 로드
			Connection conn = DriverManager.getConnection("jdbc:mysql://192.168.23.148:33060/kopo31", "root", "kopo31");
			// mysql에 연결
			Statement stmt = conn.createStatement(); // Statement에 conn을 연결하여 Statement 생성
			...
			while (rset.next()) { // 읽어올 값이 남아있는 한 루프 반복
				...
				Gongji2 gongji = new Gongji2(); // 후보 객체 생성
	...
	@Override
	public Gongji2 selectOneGongji(int id) {
		Gongji2 select = new Gongji2(); // 공지 객체 생성

		try { // 오류를 방지하기 위한 try문 시작
			Class.forName("com.mysql.cj.jdbc.Driver"); // MySQL JDBC 드라이버 클래스를 로드
			Connection conn = DriverManager.getConnection("jdbc:mysql://192.168.23.148:33060/kopo31", "root", "kopo31"); // mysql에
																															// 연결
			Statement stmt = conn.createStatement(); // Statement에 conn을 연결하여 Statement 생성
	...
	@Override
	public int updateGongji(Gongji2 gongji) {
		...
		try { // 오류를 방지하기 위한 try문 시작
			...
			stmt.execute("update gongji2 set title='" + title + "',content='" + content + "' where id=" + id + ";"); // 후보
																														// 테이블의
			conn.close(); // Connection 닫기
			stmt.close(); // Statement 닫기
		} catch (Exception e) { // 오류를 잡았을 경우
	...
	@Override
	public int insertGongji(Gongji2 gongji) {
	...
	@Override
	public List<Gongji2> selectAll(int pageNum, int countPerPage) {
		List<Gongji2> select = new ArrayList<Gongji2>(); // 학생 정보 객체들이 모인 리스트 생성
		...
		try { // 오류를 방지하기 위한 try문
			...
			while (rset.next()) { // 읽어올 값이 있는 한 루프 반복
				...
				Gongji2 gongji = new Gongji2(); // StudentScore 객체 생성
	...
	@Override
	public int getPageById(int id, int countPerPage) {
	...
	public int deleteData() {
		try { // 오류를 방지하기 위한 try문 시작
			...
			stmt.execute("delete from gongji2 where id > 0;"); // 테이블에 존재하는 모든 데이터 삭제
			conn.close(); // Connection 닫기
			stmt.close(); // Statement 닫기
		} catch (Exception e) { // 오류를 잡았을 경우
			e.printStackTrace(); // 오류 내용 프린트
