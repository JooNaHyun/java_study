...
body {
	...
	margin: 0;
	padding: 20px;
	background-color: #f2f2f2; /* 본문의 배경색 설정 */
}
...
th {
	background-color: #66cccc; /* 테이블 헤더 셀의 배경색 설정 */
	color: #fff; /* 테이블 헤더 셀의 텍스트 색상 설정 */
}

tr:nth-child(even) {
	background-color: #e5f1f1; /* 테이블에서 짝수 행의 배경색 설정 */
}
...
<body>
	...
	out.println("<th>12시간 예상 강수량(mm)</th>"); // 12시간 예상 강수량 입력
	out.println("<th>12시간 예상 적설량(cm)</th>"); // 12시간 예상 적설량 입력
	...
	out.println("<th>6시간 예상 강수량(mm)</th>"); //6시간 예상 강수량 입력
	out.println("<th>6시간 예상 적설량(cm)</th>"); //6시간 예상 적설량 입력
	%>
	<%
	for (int i = 0; i < tag_001.getLength(); i++) { // tag_001의 길이만큼 루프
		...
		if (pty.equals("0")) {
		    out.println("<td><img src='image/null.png'></td>"); // pty가 "0"일 때 null.png 이미지를 출력
